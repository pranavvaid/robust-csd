...
class StandardROIHeads(ROIHeads):
    ...
    def forward(
        ...
        """
        del images
        if self.training and not self.consistency_mode:
    ...
    def forward_with_given_boxes(
        ...
        Returns:
            list[Instances]:
                ...
                fields such as `pred_masks` or `pred_keypoints`.
        """
        assert (not self.training) or self.consistency_mode
        assert instances[0].has("pred_boxes") and instances[0].has("pred_classes")
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances]):
        ...
        """
        if not self.mask_on:
            return {} if (self.training and not self.consistency_mode) else instances
        ...
        if self.mask_pooler is not None:
            ...
            boxes = [x.proposal_boxes if self.training and not self.consistency_mode else x.pred_boxes for x in instances]
